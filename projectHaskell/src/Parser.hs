...
dict = P.parser inner where
    inner (first:rest) =
      case P.runParser (P.char 'd') (first:rest) of
        Success (_, rest) ->
          case P.runParser (P.many (P.andThen string value)) rest of
            Error _ -> Error (P.UnexpectedInput (first:rest) "not the correct input")
            Success (list, second_rest) ->
              case P.runParser (P.char 'e') second_rest of
                Error _ -> Error (P.UnexpectedInput (first:rest) "not the correct input")
                Success (_, third_rest) -> Success (list, third_rest)                       
        Error _ -> Error (P.UnexpectedInput (first:rest) "not the correct input")
