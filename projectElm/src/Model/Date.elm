module Model.Date exposing (Date, Month(..), compare, compareMonth, full, month, monthToString, monthsBetween, monthsBetweenMonths, offsetMonths, onlyYear, view, year, monthToStringM)
...
monthsBetween dA dB =
  ...
  let
    yearDifference = abs(year dA - year dB)
  in
    case (month dA, month dB) of
          -- if dA and dB don't have months specified, we calculate the months difference
          (Just m1, Just m2) -> 
                  if(year dA < year dB) then
                     if(monthToInt m1 > monthToInt m2) then
                       Just (12*yearDifference - (monthsBetweenMonths m1 m2))
                     else
                       Just (12*yearDifference + (monthsBetweenMonths m1 m2))
                  else
                     Just (abs(12*(year dA - year dB) - (monthsBetweenMonths m1 m2)))
          -- if dA and dB don't have months specified, we calculate the year difference 
          -- and multiply by 12 (12 months in a year)
...
compare (Date d1) (Date d2) =
    ...
    let
      yearDifference = d1.year - d2.year
    in
    ...
    --Debug.todo "Implement Model.Date.view"
     div [] 
       [ p [] [ text ("year: " ++ String.fromInt(d.year))],
         p [] [ text ("month: " ++ (monthToStringM d.month))]
       ]
...
monthToStringM : Maybe Month -> String
monthToStringM m =
    case m of
        Just Jan ->
            "January"
